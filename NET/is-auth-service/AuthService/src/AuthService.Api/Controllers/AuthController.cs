...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Register([FromBody] RegisterRequest request, CancellationToken cancellationToken)
    {
        ...
        // ✅ Publicar evento Kafka (fire-and-forget con manejo de error)
        try
        {
            await _kafkaProducer.PublishUserRegisteredAsync(new UserRegisteredEvent(
                UserId: Guid.NewGuid(),
                Email: request.Email,
                Username: request.Email.Contains("@")
                            ? request.Email.Substring(0, request.Email.IndexOf("@", StringComparison.Ordinal))
                            : request.Email,
        ...
        {
            // El registro ya fue exitoso, no fallar por Kafka
            _logger.LogWarning(ex, "No se pudo publicar evento UserRegistered en Kafka");
        }
